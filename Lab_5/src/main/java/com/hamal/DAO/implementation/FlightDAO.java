...
public class FlightDAO implements BasicDAO<Flight, Integer> {
    ...
    @Override
    public final  List<Flight> findAll() throws SQLException {
        List<Flight> entityList = new ArrayList<>();
        try (Session session = SessionManager.getSession()) {
            Query query = session.createQuery(getQueryManger().getFindAllQuery());
            for (Object entity : query.list()) {
                entityList.add((Flight) entity);
            }
            return entityList;
        }
    }
    ...
    @Override
    public final Flight findById(Integer id) throws SQLException {
    ...
    @Override
    public final void update(Flight entity) throws SQLException {
